...
describe('дефисные теги в формуле', () => {
  ...
  // Разделитель за командой — пустая группа, а не пробел, и это половина пары с
  // токенайзером диалекта: закрывающий доллар не может стоять за пробелом
  // (правило Pandoc), поэтому формула, которой страница кончила тег, кончалась
  // пробелом и отвергалась целиком — читателю показывали исходник файла. Пустая
  // группа отделяет команду от буквы не хуже пробела и пробелом не является.
  it('формула не кончается пробелом', () => {
    const md = asText('x <br>');
    expect(md).toBe('$x \\lt{}br\\gt{}$');
