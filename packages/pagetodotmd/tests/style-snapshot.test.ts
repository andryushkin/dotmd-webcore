...
describe('flex blockification: a row of items is not a paragraph each', () => {
  ...
  // The name is one name. `ROW_ATTR` is declared in the core and imported by
  // `style-snapshot.ts`, which used to write the same string out again under a
  // comment saying the two sides could not spell it differently — the one shape
  // where nothing fails until they do. A tag list is what proves it: the markup
  // holds no whitespace between the items, so the gap the reader saw exists only
  // in the mark, and a mark the core cannot find leaves `c#python`.
  it('the mark this side writes is the one the core reads', () => {
    const html = '<div class="row"><a class="item">c#</a><a class="item">python</a></div>';
    const { doc } = snapshot(html, CHROME);
    expect(doc.querySelector('div')!.getAttribute(ROW_ATTR)).not.toBeNull();
    const { before, after } = convert(html, CHROME);
    expect(before).toBe('c#python');
    expect(after).toBe('c# python');
  });
...
describe('the diagnostic pass does not cost the snapshot', () => {
  ...
  it('writes the same marks it writes without diagnostics', () => {
    ...
    snapshotStyles([diagnosed.body], styleEngine(TAILWIND), { diagnostics: true });
  ...
  it('and adds what it read', () => {
    const doc = page(html);
    const restore = snapshotStyles([doc.body], styleEngine(TAILWIND), { diagnostics: true });
