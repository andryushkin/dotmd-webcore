...
import { enrichRange, toMarkdown } from '../src/engine.js';
import { preservesSourceWhitespace } from '../src/engine.js';
// The sanitizer itself, reached past the package boundary on purpose. It is not
// part of `htmltodotmd`'s public surface and must not become part of it for a
// test's sake — but the pair this file holds together (`preservesSourceWhitespace`
// promises what the sanitizer does) can only be checked by running both, and a
// deep import that a rename breaks breaks *here*, loudly, which is the whole
// difference between a test doing this and shipped code doing it.
import { sanitize } from '../../htmltodotmd/src/core/sanitizer.js';
import { snapshotScope, type ComputedStyleOf } from '../src/style-snapshot.js';
import {
...
} from '../src/hard-breaks.js';
import { DEFAULT_NAMESPACE } from '../src/namespace.js';

/** The mark, under the name the default session gives it. */
const HARD_BREAK_ATTR = DEFAULT_NAMESPACE.hardBreak;
...
// Which subtrees a `<br>` must not reach, and where the answer comes from.
//
// Half of it is the core's: whatever the sanitizer will not collapse already
// draws its own newlines, so rewriting one here draws the break twice or puts a
// tag inside a code sample. That half is asked (`preservesSourceWhitespace`), and
// the two cases below hold it to its word from both ends — the predicate names
// exactly the tags whose whitespace survives a sanitize, and the rewrite declines
// exactly where the predicate says yes. The tags are never listed in the
// expectation, only in the probe: a list here would be a third copy of the set.
//
// The other half is the extension's own and is about prose rather than
// whitespace — `script`, `style`, `svg` and a maths subtree hold none.
describe('hard breaks: the whitespace model is the core\'s, asked and not copied', () => {
  // Every tag worth asking about: the five the core preserves, prose containers
  // that must keep collapsing, and `listing`, an obsolete verbatim tag neither
  // side claims — there to catch an expectation that guessed instead of asking.
  // Nothing here needs a parent of a particular kind: a `<td>` outside a table is
  // dropped by the parser before either side is asked, which measures happy-dom.
  const PROBE = [
    'pre', 'code', 'kbd', 'samp', 'textarea',
    'div', 'span', 'p', 'blockquote', 'q', 'listing',
  ];

  /** Whether the sanitizer leaves a newline inside this tag as it found it. */
  function sanitizerKeeps(tag: string): boolean {
    const doc = page(`<div><${tag}>a\nb</${tag}></div>`);
    const root = doc.querySelector('div')!;
    sanitize(root, 'selection');
    return (root.textContent ?? '').includes('\n');
  }

  /** Whether the rewrite turns that same newline into a `<br>`. */
  function rewrites(tag: string): boolean {
    const doc = page(`<div class="drawn"><${tag}>a\nb</${tag}></div>`);
    const computed = styleEngine({ drawn: 'pre-wrap' });
    const scope = doc.querySelector('.drawn')!;
    const unmark = markPreservedNewlines(elementsPreservingNewlines([scope], computed));
    try {
      const fragment = enrichRange(contentsOf(doc, '.drawn'));
      breakPreservedNewlines(fragment, false);
      return fragment.querySelectorAll('br').length > 0;
    } finally {
      unmark();
    }
  }

  it('the predicate names the tags the sanitizer really leaves alone', () => {
    for (const tag of PROBE) {
      expect([tag, preservesSourceWhitespace(tag)]).toEqual([tag, sanitizerKeeps(tag)]);
    }
  });

  it('and the rewrite declines in exactly those, under a style that draws lines', () => {
    for (const tag of PROBE) {
      expect([tag, rewrites(tag)]).toEqual([tag, !preservesSourceWhitespace(tag)]);
    }
  });

  // The extension's own half. A `<br>` in any of these is not a line the reader
  // saw: two of them the sanitizer deletes whole, and the rest are read by a rule
  // that wants the renderer's text character for character.
  it('no break reaches a script, a style, an svg or a maths subtree', () => {
    const doc = page(
      '<div class="drawn">' +
      '<script>a\nb</script><style>a\nb</style>' +
      '<svg><text>a\nb</text></svg>' +
    ...
    const unmark = markPreservedNewlines(
      elementsPreservingNewlines([doc.querySelector('.drawn')!], computed),
    );
    try {
      const fragment = enrichRange(contentsOf(doc, '.drawn'));
      breakPreservedNewlines(fragment, false);
      const breaks = Array.from(fragment.querySelectorAll('br'));
      // One, and it is the one the reader met: the `<span>` outside all of them.
      expect(breaks.length).toBe(1);
      expect(breaks[0]!.parentElement!.tagName.toLowerCase()).toBe('span');
    } finally {
      unmark();
    }
  });
