...
import { toMarkdown, enrichRange } from '../src/engine.js';
import { snapshotScope, snapshotStyles, type ComputedStyleOf } from '../src/style-snapshot.js';
import {
...
} from '../src/shadow-selection.js';
import { DEFAULT_NAMESPACE } from '../src/namespace.js';

// The copy is a `<span>` carrying the session's mirror mark, not an element
// named after the extension: a hyphenated tag name is a custom element name, so
// `createElement` of one runs whatever constructor the page registered under it.
// happy-dom models neither custom elements' side effects nor slot assignment
// (see the note at the top of this file), so what is checked here is the mark —
// that the copies are planted, and that every one of them comes back off.
const MIRROR = `[${DEFAULT_NAMESPACE.shadowMirror}]`;
...
describe('shadow selection: a closed shadow root', () => {
  ...
  it('plants no copy for it', () => {
    ...
    expect(page.doc.querySelectorAll(MIRROR)).toHaveLength(0);
    undo();
  });
});
...
describe('shadow selection: the shadow copies', () => {
  it('lists nested roots deepest first, so an inner copy is serialised into the outer', () => {
    ...
    const undo = mirrorShadowRoots(openShadowRoots(page.doc));
    try {
      const outer = page.host.querySelector(MIRROR)!;
  ...
  // The copy is planted in the *light* DOM, where a `<slot>` is what draws it —
  // and a default `<slot>` draws every unassigned light child, which is what the
  // commonest web component there is has. The copy therefore asks for a slot by
  // name, and the name is refused where the shadow tree happens to declare it.
  //
  // happy-dom does not model slot assignment (see the note at the top of this
  // file), so what these two hold is the rule and not the drawing: the copy
  // always claims a slot, and never one this root offers. The last word is a
  // component with a default slot in Chrome — case Q6 of
  // `docs/test_conversion_spec_page.html`.
  it('claims a slot, so a default `<slot>` has nothing to assign it to', () => {
    const page = pageWithComponent('<slot></slot><p>drawn</p>');
    const undo = mirrorShadowRoots(openShadowRoots(page.doc));
    try {
      const copy = page.host.querySelector(MIRROR)!;
      expect(copy.getAttribute('slot')).toBe(DEFAULT_NAMESPACE.shadowSlot);
      // A `<span>` and not a hyphenated name: `createElement('a-b')` runs
      // whatever constructor the page registered under it, and a built-in tag
      // cannot be a custom element.
  ...
  // Not decoration: the tag the copy is made of decides what the text either side
  // of it looks like, and the two available answers differ. A `<span>` hands its
  // content back to the run around it; anything the converter does not know is
  // opaque, and the space before the copy and the space after its content were
  // then written as two. A wrapper the capture invented must not be able to do
  // that — the page did not write it.
  it('is a wrapper the text reads through, which an unknown tag is not', () => {
    const window = new Window();
    const doc = window.document as unknown as Document;
    const convert = (inner: string): string => {
      doc.body.innerHTML = `<p>${inner}</p>`;
      return toMarkdown(doc.querySelector('p')! as unknown as Node);
    };
    expect(convert('<span>a </span> b')).toBe('a b\n');
    expect(convert('<s2md-shadow>a </s2md-shadow> b')).toBe('a  b\n');
  });

  it('takes a different slot when the component declares the one it wanted', () => {
    const wanted = DEFAULT_NAMESPACE.shadowSlot;
    const page = pageWithComponent(`<slot name="${wanted}"></slot><p>drawn</p>`);
    const undo = mirrorShadowRoots(openShadowRoots(page.doc));
    try {
      const claimed = page.host.querySelector(MIRROR)!.getAttribute('slot');
      expect(claimed).not.toBe(wanted);
      expect(claimed).not.toBeNull();
      expect(page.root.querySelector(`slot[name="${claimed}"]`)).toBeNull();
    } finally {
      undo();
    }
  });
  ...
  it('takes every copy back off, innermost included', () => {
    ...
    const undo = mirrorShadowRoots(openShadowRoots(page.doc));
    expect(page.doc.querySelectorAll(MIRROR).length).toBeGreaterThan(0);
    undo();
    expect(page.doc.body.innerHTML).toBe(before);
    expect(page.root.querySelectorAll(MIRROR)).toHaveLength(0);
  });
  ...
  it('hands back an undo for the copies it did plant when one of them faults', () => {
    ...
    expect(page.doc.querySelectorAll(MIRROR)).toHaveLength(1);
    undo();
    expect(page.doc.querySelectorAll(MIRROR)).toHaveLength(0);
  });
});
