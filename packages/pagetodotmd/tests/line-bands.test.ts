...
import { ONE_LINE_MARK, ROW_ATTR } from '../src/engine.js';
import {
  NOTHING_MEASURED,
  contentRectsIn,
  snapshotStyles,
  ...
  type ContentRectsOf,
  type DrawnRect,
} from '../src/style-snapshot.js';
import { cloneRangeWithBr } from '../src/capture.js';
...
function convert(html: string, rects: ContentRectsOf = statedRects): string {
  const doc = page(html);
  const restore = snapshotStyles([doc.body], styleEngine(), { contentRects: rects });
...
function rowMarks(html: string): string[] {
  const doc = page(html);
  const restore = snapshotStyles([doc.body], styleEngine(), { contentRects: statedRects });
...
describe('content drawn on one band (theory C)', () => {
  ...
  describe('the mention in the sentence', () => {
    ...
    it('is what it was where the measurement faults', () => {
      ...
      // the capture nothing.
      const doc = page(MENTION);
      const restore = snapshotStyles([doc.body], styleEngine(), { contentRects: contentRectsIn(doc) });
      const md = toMarkdown(doc.body).trim();
      restore();
