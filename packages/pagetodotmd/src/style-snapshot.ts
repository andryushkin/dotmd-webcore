 ...
 *    page may own the name — the same discipline `core/packages/htmltodotmd/src/browser.ts` uses for
...
// The mark left on a container whose items the reader saw side by side is
// `namespace.row`, and the recorded style is `namespace.style`. Neither name is
// spelled in this file: the session carries them, and its default takes both
// from the converter's own declarations, so a rename there is a rename here
// rather than a silent disagreement (`namespace.ts`). Why it is one mark on the
// container rather than a `display` on each item is stated where the converter
// declares it; it is only written here, and this file used to spell the string a
// second time under a comment saying the two sides could not disagree about it.
 ...
 */
// The name is `namespace.diagnostic`, taken from the session rather than spelled
// here: every attribute this file writes lands on somebody else's live page, and
// a second consumer doing the same at the same time is what the namespace exists
// to keep apart (`namespace.ts`).
...
/** What a snapshot needs beyond the nodes and a way to read their style. */
export interface SnapshotOptions {
  /** Write down what was read beside each verdict. One attribute per element. */
  diagnostics?: boolean;
  /**
   * How to measure the lines a container's content was drawn on. Absent means
   * unmeasured, which is the ordinary case away from a browser — and there the
   * capture is exactly what it was before any of this existed.
   */
  contentRects?: ContentRectsOf;
  /** The attribute names this capture may write on the page (`namespace.ts`). */
...
export function snapshotStyles(
  ...
  { diagnostics = false, contentRects = NOTHING_MEASURED, namespace = DEFAULT_NAMESPACE }:
    SnapshotOptions = {},
): () => void {
  const styleAttr = namespace.style;
  const rowAttr = namespace.row;
  const diagnosticAttr = namespace.diagnostic;
  ...
  const record = (
    ...
    if (declarations.length === 0 && el.getAttribute(styleAttr) === null) return undefined;
  ...
  try {
    for (const entry of pending) {
      ...
      const previous = entry.el.getAttribute(styleAttr);
      if (text === '' && previous === null) continue;
      undo.push(
        previous === null
          ? () => entry.el.removeAttribute(styleAttr)
          : () => entry.el.setAttribute(styleAttr, previous),
      );
      if (text === '') entry.el.removeAttribute(styleAttr);
      else entry.el.setAttribute(styleAttr, text);
    }
  } catch {
  ...
  try {
    for (const { el, text } of diagnosed) {
      if (text === '') continue;
      const previous = el.getAttribute(diagnosticAttr);
      undo.push(
        previous === null
          ? () => el.removeAttribute(diagnosticAttr)
          : () => el.setAttribute(diagnosticAttr, previous),
      );
      el.setAttribute(diagnosticAttr, text);
    }
  } catch {
  ...
  try {
    for (const { el, mark } of rows) {
      ...
      const previous = el.getAttribute(rowAttr);
      undo.push(
        previous === null
          ? () => el.removeAttribute(rowAttr)
          : () => el.setAttribute(rowAttr, previous),
      );
      el.setAttribute(rowAttr, mark);
    }
  } catch {
    /* same: every attribute already written is in `undo` */
  }
...
/**
 * The part of the page a capture rooted at this element can reach.
 *
 * The element itself, except that one inside a table is given back its header row
 * from above it (`enrichRange`) — so anything in a table is snapshotted from the
 * table down. Nothing else in the capture reads outside its own root, and walking
 * the whole document instead would spend the cost of every element on a page to
 * convert a paragraph of it.
 *
 * Every path that hands a scope to `snapshotStyles()` asks this, and none of them
 * spells the step out: `highlightsToMd()` had its own `closest('table')` beside
 * the one below, so the reason for the reach was written twice and could be
 * repaired once.
 */
export function snapshotScopeOf(el: Element | null): Element | null {
  return el?.closest?.('table') ?? el;
}

/** The same question asked of a range, which answers for its common ancestor. */
export function snapshotScope(range: Range): Element | null {
  const node = range.commonAncestorContainer;
  return snapshotScopeOf(node.nodeType === 1 ? (node as Element) : node.parentElement);
}
