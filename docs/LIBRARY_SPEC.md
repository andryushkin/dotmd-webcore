...
> Ошибки whitespace ломают весь markdown-вывод: это фундамент, от которого
> зависят все остальные модули.
...
> Пользователь выделяет произвольный фрагмент на странице.
...
> Таблицы — самый сложный элемент. Нужна трёхуровневая стратегия.
...
> Основная сложность — lazy-load и `<picture>`.
...
> Главная сложность — определение языка и line numbers.
...
> Сноски — расширение Markdown, не везде поддерживаются.
